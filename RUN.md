...
1. 部署者准备数据库、连接凭据与存储；启动前应用全部迁移至 `021_midi_slug_history.sql`（Compose 由 migrate 服务执行）。安装页不创建数据库、不自动迁移或 seed。
...
**当前 Vercel + Neon 已有库：先迁移，再更新应用。** 发布当前工作区前，先核对生产的 `schema_migrations`，用已有幂等迁移器应用全部待执行迁移至 021；不要仅根据历史发布记录推断线上版本。先完成隔离恢复演练，再按下列步骤更新应用。即使 `MIDI_IMPORT_ENABLED=false`，也不要跳过迁移。
...
公开搜索可按作品标题、slug、人物姓名、历史昵称和网站名称查询；`/search` 结果不进入 Sitemap。Sitemap 按静态入口、MIDI、人物和文章分片，每片最多一万条，robots.txt 列出各分片；内容页使用真实更新时间。前端 Web Analytics 组件已接入根布局，部署后在 Vercel 项目中启用并访问生产站点验证采集。
