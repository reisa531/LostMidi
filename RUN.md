...
适用版本：当前仓库的一次性安装、PostgreSQL 搜索、可恢复删除与审计、历史来源评级和受保护证据附件，以及默认关闭的管理员单文件 MIDI 导入（local / S3）。导入已实现并对真实 S3 桶完成联调验证；生产启用需配置 S3 变量并确认公开分发。
...
生产架构为 **Vercel 容器 + Neon Free** 后端及独立 Vercel 前端项目。当前工作区包含迁移至 018；发布前须按第 7.2 节核对实际线上版本、备份并安全迁移已有库，不能以空库安装流程替代。历史部署记录中的迁移编号不代表当前线上状态。
...
后台支持多个超级管理员与管理员账号，账号来自首次安装或超级管理员发出的 48 小时邀请；访问者无需账号，只能浏览公开内容。超级管理员可管理账号并直接发布；普通管理员提交内容变更进入审核，获批才会发布，涵盖基础资料、署名、来源/寻回、证据、文件、删除和恢复。对外部署需使用 HTTPS 和 Secure Cookie。完整单机模式下 PostgreSQL 与后端不需要直接暴露到公网，`noindex` 仅控制索引。
...
1. 部署者准备数据库、连接凭据与存储；启动前应用全部迁移至 `018_freeform_source_type_labels.sql`（Compose 由 migrate 服务执行）。安装页不创建数据库、不自动迁移或 seed。
...
本分支当前应用需要全部迁移至 `018_freeform_source_type_labels.sql`。009 增加 PostgreSQL trigram 搜索索引；010 增加回收站和审计；011 增加对象清理重试信息；012 增加来源类型、人工可信度、核验时间和数据库内证据附件；013 增加多管理员及邀请；014 为作品和人物增加稳定公开 UUID；015 增加内容审核队列；016 增加人物资料和推测日期；017–018 支持来源类型多选及自定义文本标签。证据附件为最多 1 MiB 的 PDF、JPEG、PNG 或纯文本，SHA-256 校验，下载须后台会话。关闭文件上传也不能跳过后续迁移。
...
**当前 Vercel + Neon 已有库：先迁移，再更新应用。** 发布当前工作区前，先核对生产的 `schema_migrations`，用已有幂等迁移器应用全部待执行迁移至 018；不要仅根据历史发布记录推断线上版本。先完成隔离恢复演练，再按下列步骤更新应用。即使 `MIDI_IMPORT_ENABLED=false`，也不要跳过迁移。
...
| Backend 启动失败或 /ready 503 | 查看 backend、migrate、postgres 日志；核对数据库迁移已完整应用至 015，以及审核、用户邀请、公开 UUID、证据和既有搜索、回收站、revision、会话与安装结构；不要将数据库故障当作未安装 |
...
本手册说明部署契约与维护步骤。用户已确认阶段 1–3 手动部署完成，数据库迁移到 011；阶段 4–6 在本分支完成，发布前须升级已有数据库至 015。此前数据库备份为 `.tools/production-before-release-cd9ea75.dump`，不包含 S3 对象。上线前需使用独立空库运行恢复演练脚本，并单独确认文件存储备份。
...
在专用测试环境执行现有 `scripts/midi_import_smoke.py --api <测试后端> --allow-writes`，以及 `scripts/midi_import_browser_smoke.py --frontend <测试前端> --allow-writes --screenshots <输出目录>`。浏览器脚本覆盖上传与下载、失败保留、响应丢失重试、五模块导航、状态筛选、分页、分组及桌面/手机布局。关闭同一测试后端的导入后，可追加 `--expect-disabled` 验证文件输入隐藏及纯资料建档；脚本使用环境变量中的测试管理员凭据。
