...
export default async function PeoplePage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return <>
    ...
    <form method="get" className="mb-6 flex flex-wrap gap-2"><input type="search" name="q" defaultValue={query.q ?? ""} maxLength={200} aria-label="搜索人物姓名或历史昵称" placeholder="搜索姓名或历史昵称" className="min-w-0 flex-1 rounded-lg border border-line bg-white px-3 py-3 text-sm" /><button className="rounded-lg bg-accent px-5 py-3 text-sm text-white">搜索人物</button>{query.q && <Link href="/people" className="self-center text-sm underline">清除</Link>}</form>
    ...
    {result.data.length ? <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{result.data.map(person => <article key={person.id} className="flex min-w-0 flex-col rounded-xl border border-line bg-white/70 p-5 [overflow-wrap:anywhere]">
      ...
      <h3 className="font-serif text-xl"><Link href={`/people/${person.public_id}`} className="hover:text-accent hover:underline">{person.display_name}</Link></h3>
      <p className="mt-2 text-xs leading-6 text-muted">历史昵称：{person.aliases.length ? person.aliases.join(" / ") : "尚未登记"}</p>
      <p className="mb-5 mt-3 line-clamp-3 text-sm leading-7 text-muted">{markdownSummary(person.summary || person.biography || "人物资料尚待补充。已知署名与相关作品保留在人物档案中。", 220)}</p>
      <div className="mt-auto flex flex-wrap justify-between gap-3 border-t border-line pt-4 text-xs"><Link href={`/people/${person.public_id}`} className="archive-link" aria-label={`查看 ${person.display_name} 的人物档案`}>人物详情 →</Link><Link href={`/search?person=${encodeURIComponent(person.id)}`} className="text-muted hover:text-accent hover:underline" aria-label={`搜索 ${person.display_name} 的相关作品`}>相关作品 →</Link><Link href={catalogHref("/map", { group: person.id })} className="text-muted hover:text-accent hover:underline" aria-label={`按 ${person.display_name} 查看作品分组`}>查看作品分组</Link></div>
