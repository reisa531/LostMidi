...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  try {
    ...
    return { title, description, alternates: { canonical: `/people/${detail.person.public_id}` }, openGraph: { type: "profile", title, description } };
...
export default async function PersonPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <article className="mx-auto max-w-3xl"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData).replace(/</g, "\\u003c") }} /><p className="eyebrow">档案人物</p>
    ...
    <Section title="相关作品">{detail.midis.length ? <ul className="space-y-4">{detail.midis.map(m => <li key={`${m.id}-${m.role}`}>
      <Link className="archive-link" href={`/midis/${m.public_id}`}>{m.title}</Link><span className="ml-3 text-muted">{roleName(m.role)}</span>
