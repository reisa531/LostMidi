...
export default async function MapPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return <>
    <PageHeader eyebrow="Archive / Map" title="Map · 档案分组" description="不是地理地图，而是一份作品与来处的索引。按作者或历史网站归拢记录，查看各组的文件留存与下载权限情况。" />
    ...
    <section id="group-entries" aria-labelledby="group-title" className="scroll-mt-6"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div className="min-w-0"><p className="eyebrow">Group / Entries</p><h2 id="group-title" className="mt-2 font-serif text-2xl [overflow-wrap:anywhere]">{selected ? selectedLabel : "选择一个分组"}</h2></div>{selected && <Link className="archive-link text-xs" href={catalogHref("/map", clearSelection)}>取消分组选择</Link>}</div>
