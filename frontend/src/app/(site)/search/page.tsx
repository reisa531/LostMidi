...
export default async function SearchPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return <>
    ...
    {!q && !personId ? <EmptyState title="输入关键词开始搜索" description="支持作品名称、slug、人物姓名、历史昵称和历史来源。" /> : <>
      {personId && selectedPerson && <p className="mb-4 rounded-lg border border-line bg-white/70 p-3 text-sm">人物：{selectedPerson.person.display_name} {q && `· 关键词：“${q}”`} <Link className="archive-link ml-3" href={q ? `/search?q=${encodeURIComponent(q)}` : "/search"}>清除人物筛选</Link></p>}
      ...
      <section className="mb-8"><h2 className="mb-3 font-serif text-xl">作品 <span className="font-sans text-sm text-muted">{entries?.pagination.total ?? 0}</span></h2>
        {entries?.data.length ? <ul className="divide-y divide-line rounded-xl border border-line bg-white/70 px-5">{entries.data.map(entry => <li key={entry.id} className="py-4"><Link className="font-medium hover:text-accent hover:underline" href={`/midis/${entry.slug}`}>{entry.title}</Link><p className="mt-1 text-xs text-muted">{entry.slug} · {entry.credits.map(item => item.display_name).join("、") || "署名待补"}</p></li>)}</ul> : <EmptyState title={midiPage > midiPages ? "作品页码超出范围" : "没有匹配的作品"} action={midiPage > midiPages ? <Link className="archive-link" href={href("midiPage", 1)}>返回作品第一页</Link> : undefined} />}
