...
export default async function SearchPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
    ...
    try {
      if (personId) {
        [entries, selectedPerson] = await Promise.all([
          getCatalogEntries({ page, pageSize: 10, sort: "updated", person: personId, q }), getPersonById(personId),
        ]);
      } else [entries, people] = await Promise.all([
        getCatalogEntries({ page, pageSize: 10, sort: "updated", q }), getCatalogPeople({ page, pageSize: 10, q }),
      ]);
    } catch (error) {
  ...
  const pages = Math.max(1, Math.ceil((personId ? entries?.pagination.total ?? 0 : Math.max(entries?.pagination.total ?? 0, people?.pagination.total ?? 0)) / 10));
  const href = (nextPage: number) => `/search?${new URLSearchParams({ ...(q ? { q } : {}), ...(personId ? { person: personId } : {}), page: String(nextPage) })}`;
  return <>
