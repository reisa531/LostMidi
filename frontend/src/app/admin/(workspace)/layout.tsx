...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#f2f4f0] lg:grid lg:grid-cols-[240px_minmax(0,1fr)]">
    <aside className="bg-[#203d31] px-5 py-5 text-white lg:sticky lg:top-0 lg:flex lg:h-screen lg:flex-col lg:py-8">
      <Link href="/admin" className="mb-6 flex min-w-0 items-center gap-3 px-3 lg:mb-12"><span aria-hidden="true" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-white/25 font-serif text-2xl">♮</span><span className="min-w-0"><span className="block text-sm font-semibold [overflow-wrap:anywhere]">{site.name}</span><span className="mt-1 block text-[10px] tracking-[0.18em] text-[#bdcec5]">ADMINISTRATION</span></span></Link>
