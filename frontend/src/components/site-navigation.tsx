...
export function SiteNavigation() {
  ...
  useEffect(() => {
    ...
    return () => { desktop.removeEventListener("change", closeOnDesktop); restoreScrolling(); };
  }, []);
  ...
  return <>
    ...
    <button ref={trigger} type="button" onClick={() => { previousOverflow.current = document.body.style.overflow; document.body.style.overflow = "hidden"; dialog.current?.showModal(); }} aria-haspopup="dialog" aria-controls="mobile-site-navigation" className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-line bg-white px-3 text-sm font-medium text-accent lg:hidden">
      <span aria-hidden="true" className="text-xl leading-none">☰</span>菜单
    </button>
    <dialog ref={dialog} id="mobile-site-navigation" aria-label="网站导航" onClose={() => { restoreScrolling(); if (trigger.current?.getClientRects().length) trigger.current.focus(); }} onClick={event => { if (event.target === dialog.current) dialog.current.close(); }} className="fixed inset-0 ml-auto mr-0 h-dvh max-h-none w-[min(22rem,88vw)] max-w-none overflow-y-auto overscroll-contain border-l border-line bg-background p-0 text-foreground shadow-2xl backdrop:bg-black/45 lg:hidden">
