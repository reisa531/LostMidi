...
export function MidiSectionNav({ id, slug, current }: { id: string; slug: string; current: typeof sections[number][0] }) {
  return <nav aria-label="作品资料管理" className="mb-6 flex flex-wrap gap-x-5 gap-y-3 border-b border-line pb-4 text-sm">
    ...
    <Link className="archive-link" href={`/midis/${slug}`} target="_blank" rel="noopener noreferrer">公开详情 ↗</Link>
