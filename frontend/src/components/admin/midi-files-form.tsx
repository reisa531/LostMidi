...
export function MidiFilesForm({ midiId, revision, maxFileSize, enabled }: {
  ...
  return <form action={action} onReset={event => event.preventDefault()} onSubmit={event => {
    ...
    <p className="text-sm leading-7 text-muted">仅供私下归档，不代表获得公开分发许可，不会修改作品的归档状态或权利字段。文件、署名、来源与基础资料共用版本，请勿同时在其他页面修改。</p>
    ...
    <fieldset disabled={busy || !enabled} className="min-w-0 space-y-5 disabled:opacity-60">
      <legend className="sr-only">私下归档文件</legend>
      ...
      <label className="flex items-start gap-3 text-sm leading-7"><input ref={rightsInput} type="checkbox" name="rights_confirmed" value="true" required className="mt-2 shrink-0" />我确认有权将此文件私下归档；此确认不授予或承诺公开分发许可。</label>
    </fieldset>
    ...
    {!state.error && state.result && <p role="status" className="rounded-lg bg-green-50 p-4 text-sm leading-7 text-green-900">
      {state.result.duplicate ? "当前档案已存在相同文件，已去重，未重复新增。" : "文件已成功私下归档。"}
    ...
    <div className="flex flex-wrap items-center gap-5 border-t border-line pt-5">
      <button type="submit" disabled={busy || !enabled} className="rounded bg-accent px-6 py-3 text-sm text-white disabled:opacity-50">{pending ? "正在导入，请勿重复操作…" : "确认并私下归档"}</button>
