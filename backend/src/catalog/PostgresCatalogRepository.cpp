...
const std::string entryColumns =
    "m.id,m.slug,m.title,m.estimated_year,m.archive_status,"
    "to_char(m.updated_at AT TIME ZONE 'UTC','YYYY-MM-DD\"T\"HH24:MI:SS.US\"Z\"') AS updated_at";
const std::string entryWhere =
    " FROM midi_entries m WHERE ($1::text='' OR m.archive_status=$1) "
    "AND ($2::bigint=0 OR EXISTS(SELECT 1 FROM midi_credits c WHERE c.midi_id=m.id AND c.person_id=$2)) "
    "AND ($3::text='' OR EXISTS(SELECT 1 FROM historical_sources s WHERE s.midi_id=m.id AND s.website_name COLLATE \"C\"=$3)) "
    "AND (NOT $4::boolean OR NOT EXISTS(SELECT 1 FROM midi_credits c WHERE c.midi_id=m.id)) "
...
void enrich(Snapshot& snapshot, std::vector<CatalogEntry>& entries, std::vector<CatalogEntry>* other = nullptr) {
    ...
    for (const auto& row : snapshot.exec(
        ...
        "JOIN people p ON p.id=c.person_id WHERE c.midi_id=ANY($1::bigint[]) "
...
std::string memberships(const std::string& by) {
    if (by == "author") return
        ...
        "WHERE NOT EXISTS(SELECT 1 FROM midi_credits c WHERE c.midi_id=m.id)) ";
    return
        ...
        "WHERE NOT EXISTS(SELECT 1 FROM historical_sources s WHERE s.midi_id=m.id)) ";
}
}
...
PageResult<CatalogEntry> PostgresCatalogRepository::entries(const EntryQuery& query) {
    ...
    result.total = snapshot.exec("SELECT count(*) AS total" + entryWhere,
        status, person, source, query.missingAuthor, query.missingSource)[0]["total"].as<std::int64_t>();
    const std::string order = query.sort == "title" ? "m.title COLLATE \"C\" ASC,m.id ASC" : "m.updated_at DESC,m.id DESC";
    for (const auto& row : snapshot.exec("SELECT " + entryColumns + entryWhere + "ORDER BY " + order + " LIMIT $6 OFFSET $7",
        status, person, source, query.missingAuthor, query.missingSource, static_cast<std::int64_t>(query.page.size), query.page.offset()))
...
Overview PostgresCatalogRepository::overview() {
    ...
    const auto rows = snapshot.exec(
        ...
        "(SELECT count(*) FROM people) AS people,(SELECT count(*) FROM midi_files) AS files,"
        "(SELECT count(DISTINCT website_name COLLATE \"C\") FROM historical_sources) AS sources,"
        ...
        "count(*) FILTER(WHERE m.archive_status='uncertain') AS uncertain FROM midi_entries m");
    ...
    for (const auto& recent : snapshot.exec("SELECT " + entryColumns + " FROM midi_entries m ORDER BY m.updated_at DESC,m.id DESC LIMIT 6"))
        result.recent.push_back(entryFrom(recent));
    for (const auto& attention : snapshot.exec("SELECT " + entryColumns + " FROM midi_entries m "
        "WHERE m.archive_status!='archived' ORDER BY m.updated_at DESC,m.id DESC LIMIT 6"))
...
PageResult<Person> PostgresCatalogRepository::people(Page page) {
    Snapshot snapshot(db_, observer_);
    PageResult<Person> result;
    result.page = page;
    result.total = snapshot.exec("SELECT count(*) AS total FROM people")[0]["total"].as<std::int64_t>();
    for (const auto& row : snapshot.exec("SELECT id,display_name,biography FROM people ORDER BY display_name COLLATE \"C\",id LIMIT $1 OFFSET $2",
    ...
    if (!result.data.empty()) {
        ...
        for (const auto& row : snapshot.exec("SELECT person_id,count(DISTINCT midi_id) AS midis FROM midi_credits WHERE person_id=ANY($1::bigint[]) GROUP BY person_id", ids))
...
PageResult<Group> PostgresCatalogRepository::groups(const GroupQuery& query) {
    ...
    for (const auto& row : snapshot.exec(members +
        ...
        "count(*) FILTER(WHERE EXISTS(SELECT 1 FROM midi_files f WHERE f.midi_id=m.id)) AS with_files,"
        "count(*) FILTER(WHERE EXISTS(SELECT 1 FROM midi_files f WHERE f.midi_id=m.id AND " + downloadable + ")) AS downloadable "
        "FROM membership g JOIN midi_entries m ON m.id=g.midi_id GROUP BY g.key,g.label "
