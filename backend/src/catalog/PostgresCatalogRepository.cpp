...
PageResult<Person> PostgresCatalogRepository::people(const PersonQuery& query) {
    ...
    for (const auto& row : snapshot.exec("SELECT id,display_name,biography FROM people p WHERE p.deleted_at IS NULL AND ($1::text='' OR p.display_name ILIKE $1 ESCAPE '!' "
