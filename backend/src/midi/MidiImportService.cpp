...
FileImportResult MidiImportService::import(std::int64_t id, std::int64_t revision, const std::string& filename,
    ...
    if (!enabled_) throw ApiError(503, "IMPORT_DISABLED", "File import is disabled until private durable storage is configured.");
    if (id < 1 || revision < 1) throw ApiError(400, "INVALID_INPUT", "A positive MIDI id and revision are required.");
    if (!rightsConfirmed) throw ApiError(400, "RIGHTS_CONFIRMATION_REQUIRED", "Confirm the right to privately archive this file.");
