...
protected:
    ...
    void SetUp() override {
        ...
        db_->execSqlSync("CREATE TABLE admin_users (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), username TEXT UNIQUE, email TEXT, password_hash TEXT, role TEXT, status TEXT)");
        db_->execSqlSync("CREATE FUNCTION allocate_archive_id(TEXT) RETURNS BIGINT LANGUAGE SQL AS $$ SELECT 1::BIGINT $$");
        db_->execSqlSync("CREATE TABLE admin_invitations (token_hash TEXT)");
        db_->execSqlSync("CREATE TABLE admin_change_requests (id UUID, entity_public_id UUID)");
...
TEST_F(InstallationPostgres, ReadinessRequiresLedgerAndQueryableTableAndStatusPropagatesFailure) {
    ...
    db_->execSqlSync("DELETE FROM schema_migrations WHERE version='008_deleted_creation_receipts.sql'");
    expectApiError([&] { requireDatabaseReady(db_); }, 503, "DATABASE_NOT_READY");
    db_->execSqlSync("INSERT INTO schema_migrations VALUES('008_deleted_creation_receipts.sql')");
    db_->execSqlSync("ALTER TABLE midi_creation_requests ALTER COLUMN midi_id SET NOT NULL");
    expectApiError([&] { requireDatabaseReady(db_); }, 503, "DATABASE_NOT_READY");
    db_->execSqlSync("ALTER TABLE midi_creation_requests ALTER COLUMN midi_id DROP NOT NULL");
